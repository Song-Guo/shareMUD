...
contract MUDsharing {
    ...
    uint expire_select = 10;
    uint expire_submit = 10;
    uint expire_rate = 10;
    ...
    //2. use input information create struct description
        _description = Description(_cpe_h,_cpe_o);
    ...
    //1. supplier can only provide an offer when the request was not closed.
        if(_time_offer > _expireTime) {
            ...
            revert("Request expired by offer");
        }
    ...
    //raise an error.
        ...
        uint _expire = _CurRequest.varTimeRequest + expire_offer + expire_select;
        if (_time_select > _expire) {
            revert("Request Expired by select_check");
        }
...
//1. only consumer can make a selection;
    ...
    function rate_supplier(bytes32 _uniqueID, address _supplierAddr, uint _rate) public {
        ...
        require(selectionMapping[_uniqueID][_supplierAddr] = true, "Only selected supplier of this transaction can be rated!");
    ...
    function ViewRate(address _supplierAddr) view public returns(uint, uint) {
