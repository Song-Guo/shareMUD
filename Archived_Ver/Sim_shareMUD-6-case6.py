...
contract_addr = '0x1A2a01c61a9cABC5a8E3ed70CfF8711626763631'
...
for i in selection:
    consumerAddr = accountList[i][0]
    consumerPK = accountList[i][1]
    result = ViewFunction("ViewRate", [supplierAddr])
    print(f'Existing rate of supplier{supplierAddr} are: \n')
    for k in result:
        UID = '0x' + k[0].hex()
        print(f'UID = {UID}, Rate = {k[1]} \n')
    TransactFunction("forgot_rate",consumerAddr,consumerPK,[curUID])
    result = ViewFunction("ViewRate", [supplierAddr])
    print(f'Existing rate of supplier{supplierAddr} are: \n')
    for k in result:
        UID = '0x' + k[0].hex()
...
with open("/Users/skoll/v6MarginTest/case6.csv","a") as csvfile:
